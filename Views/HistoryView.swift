...
struct HistoryView: View {
    ...
    private var filteredResults: [GradeResult] {
        guard !searchText.isEmpty else { return resultsStore.results }
    ...
    var body: some View {
        NavigationStack {
            ...
            .searchable(text: $searchText, prompt: "ابحث بالاسم أو الرقم")
            ...
            .sheet(isPresented: $showExportAll) {
                if let url = CSVExporter().writeToTemporaryFile(results: resultsStore.results,
            ...
            .overlay {
                if resultsStore.results.isEmpty {
                    ContentUnavailableView("لا توجد نتائج مسح بعد",
...
private struct HistoryDetailLoader: View {
    ...
    var body: some View {
        Group {
            if let image {
                ResultsView(result: result, alignedImage: image)
            } else {
    ...
    private func loadImage() {
        guard let path = result.correctedImagePath else { return }
        let url = AppDirectories.scannedImagesDirectory.appendingPathComponent(path)
