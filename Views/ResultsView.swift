...
//  (تحتاج مراجعة). يمكن للمستخدم لمس أي سؤال لتصحيحه يدوياً.
//
...
struct ResultsView: View {
    ...
    var body: some View {
            ...
            VStack(spacing: 20) {
                ...
                if let template {
                    GeometryReader { geo in
                        let imgAspect = CGFloat(alignedImage.width) /
                            CGFloat(alignedImage.height)
        ...
        .sheet(isPresented: $showExportSheet) {
            if let url = CSVExporter()
                .writeToTemporaryFile(results: [result]) {
    ...
    private var scoreHeader: some View {
        VStack(spacing: 6) {
            ...
            Text(
                "\(result.earnedPoints, specifier: "%.1f") / \(result.totalPoints, specifier: "%.0f")"
            )
            .font(
                .system(
                    size: 40,
            ...
            if result.ambiguousCount > 0 {
                ...
                Label(
                    "\(result.ambiguousCount) إجابة تحتاج مراجعة يدوية",
    ...
    private var studentInfoSection: some View {
        ...
        // iOS 16 compatible
        .onReceive(Just(result.studentName)) { _ in
            persist()
        }

        .onReceive(Just(result.studentId)) { _ in
            persist()
        }
    }
    ...
    private var answersListSection: some View {
        ...
        .confirmationDialog(
            ...
            if let qNum = showManualEditFor,
               let template,
               let group = template.questionGroups.first(
                    where: {
                        $0.questionNumber == qNum
                    }
               ) {
    ...
    private func setManualAnswer(
        question: Int,
        label: String?
    ) {

        guard let idx =
                result.answers.firstIndex(
                    where: {
                        $0.questionNumber == question
                    }
                )
        else {
            return
        ...
        result.answers[idx].state =
            label == nil ? .blank : .filled


        if let template,
           let key = answerKeyStore.key(
                for: template.id
           ),
           let correctLabel =
                key.correctAnswers[question] {

            let maxPoints =
                template.questionGroups.first {
                    $0.questionNumber == question
                }?.points ?? 0
            ...
            result.answers[idx].isCorrect =
                isCorrect

            result.answers[idx].pointsAwarded =
    ...
    @ViewBuilder
    private func bubbleOverlay(
        for answer: DetectedAnswer,
            ...
            ForEach(
                ...
                Circle()
                    .stroke(
                        color(
                            for: answer,
                            isThisChoiceDetected: isDetected
                        ),
                        lineWidth:
                            isDetected ? 3 : 1
                    )
    ...
    private func color(
        for answer: DetectedAnswer,
        isThisChoiceDetected: Bool
    ) -> Color {

        guard isThisChoiceDetected else {
            return .clear
        }
